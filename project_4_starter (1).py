...
get_ipython().system('{sys.executable} -m pip install alphalens==0.3.6')
...
def factor_returns(pca, returns, factor_return_indices, factor_return_columns):
    ...
    print(f'indices: {factor_return_indices}')
    print(f'columns: {factor_return_columns}')
    print(f'returns length: {returns.shape[1]}')
...
def factor_cov_matrix(factor_returns, ann_factor):
    ...
    factor_cov = factor_returns.var(ddof=1)*ann_factor
    
    factor_cov_matrix = np.diag(factor_cov.values)
...
def predict_portfolio_risk(factor_betas, factor_cov_matrix, idiosyncratic_var_matrix, weights):
    ...
    common_ret = factor_betas.dot(factor_cov_matrix).dot(factor_betas.T)
    
    predicted_portfolio_risk_df = np.sqrt(weights.T.dot(common_ret + idiosyncratic_var_matrix).dot(weights))
    
    predicted_portfolio_risk = 0
    for index,value in predicted_portfolio_risk_df.iterrows():
        predicted_portfolio_risk += value[0] 
...
def mean_reversion_5day_sector_neutral_smoothed(window_length, universe, sector):
    ...
    factor = SimpleMovingAverage(inputs=[unsmoothed_factor],window_length=window_length).rank().zscore()
...
def sharpe_ratio(factor_returns, annualization_factor):
    ...
    sharpe_ratio = {}
    for factor in factor_returns.iteritems():
        sharpe_ratio[factor[0]] = ((factor[1].mean()/factor[1].std())*annualization_factor)
...
# **Answer:** 
# 
# No major change in performance as smoothing is likely to improve the sharpe ratio or decrease turnover both of which are likely to have an impact in the form of a lower volatility profile but not necessarily result in higher performance.
...
class OptimalHoldings(AbstractOptimalHoldings):
    def _get_obj(self, weights, alpha_vector):
        ...
        alpha = []
        for i in alpha_vector.iteritems():
            alpha.append(i[1])
        alpha = np.array(alpha)
        print(alpha)
        
        objective = cvx.Minimize(-alpha*weights)
...
class OptimalHoldingsRegualization(OptimalHoldings):
    def _get_obj(self, weights, alpha_vector):
        ...
        alpha = np.array(alpha)
        print(alpha)
        print(cvx.norm(weights))
        objective = cvx.Minimize(-(alpha*weights) + self.lambda_reg*cvx.norm(weights,2))       
...
class OptimalHoldingsStrictFactor(OptimalHoldings):
    def _get_obj(self, weights, alpha_vector):
        ...
        target_weights = (alpha - np.mean(alpha)) / (np.sum(np.abs(alpha)))

        eucliedean_distance = cvx.norm(weights-target_weights.reshape(-1))

        objective = cvx.Minimize(eucliedean_distance)
